...
async def create_address(
    ...
    address = await crud_address.create(
        db=session, 
        object={**payload.model_dump(), "user_id": user.id}
    )
...
async def list_addresses(
    ...
    addresses = await crud_address.get_multi(
        db=session, 
        user_id=user.id
    )
    return ResponseModel(
        message="Addresses retrieved successfully",
        data=addresses["data"] if isinstance(addresses, dict) and "data" in addresses else addresses
    )
