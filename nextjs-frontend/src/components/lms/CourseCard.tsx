    ...
    <RevealBlock delay={delay}>
      ...
      >
        ...
        <div className="p-6 border-t border-border">
          <div className="text-label font-mono text-text-muted tracking-[0.15em] mb-3">{course.instructor}</div>
          <h3 className="text-text-main font-bold text-xl leading-tight mb-2">{course.title}</h3>
          <p className="text-text-muted text-sm mb-5 line-clamp-2">{course.subtitle}</p>
          <div className="flex items-center justify-between">
            ...
            <span className="text-text-main font-bold text-lg">€{course.price}</span>
          </div>
        </div>
      </Link>
    </RevealBlock>
        ...
        >
          ...
          <div className="p-6 border-t border-border">
            <div className="flex items-center gap-2 mb-2">
              <div className="flex gap-0.5">{Array.from({ length: 5 }).map((_, j) => <Star key={j} size={10} fill={j < Math.floor(course.rating) ? "var(--color-gold)" : "none"} className={j < Math.floor(course.rating) ? "text-gold" : "text-border"} />)}</div>
              <span className="text-text-muted text-label font-mono">{course.rating} ({course.students.toLocaleString()})</span>
            </div>
            <h3 className="text-text-main font-bold text-lg mb-1">{course.title}</h3>
            <p className="text-text-muted text-sm mb-5 line-clamp-2">{course.subtitle}</p>
            ...
            <div className="flex items-center justify-between border-t border-border pt-4">
              <span className="text-text-main font-bold text-2xl">€{course.price}</span>
