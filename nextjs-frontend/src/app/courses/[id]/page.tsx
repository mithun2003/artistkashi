    ...
    <main className="pt-24 min-h-screen">
      ...
      <div className="relative h-[55vh] min-h-[400px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-8 lg:px-16 pb-14 max-w-360 mx-auto">
          <div className="text-label font-mono text-gold tracking-[0.2em] uppercase mb-4">Masterclass</div>
          ...
          <div className="flex items-center gap-6 text-sm font-mono text-text-muted">
            <span className="flex items-center gap-1.5"><Star size={12} fill="var(--color-gold)" className="text-gold" />{course.rating}</span>
      ...
      <div className="max-w-360 mx-auto px-8 lg:px-16 pt-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-8">
            <RevealBlock>
              <div className="mb-16">
                <h2 className="text-text-main font-bold text-3xl mb-6">About This Course</h2>
            ...
            <RevealBlock>
              <div className="mb-16">
                <h2 className="text-text-main font-bold text-3xl mb-8">Curriculum</h2>
                <div className="border border-border">
                  {CURRICULUM.map((s, i) => (
                    <div key={i} className="border-b border-border last:border-b-0">
                      ...
                      >
                        <div>
                          <div className="text-label font-mono text-gold tracking-widest mb-1">Section {i + 1}</div>
                          <span className="text-text-main font-semibold">{s.section}</span>
                        </div>
                        <div className="flex items-center gap-4">
                          <span className="text-label font-mono text-text-muted">{s.lessons.length} lessons</span>
                          {openSection === i ? <Minus size={14} className="text-text-muted" /> : <Plus size={14} className="text-text-muted" />}
                        </div>
                      </button>
                      <AnimatePresence>
                        {openSection === i && (
                              ...
                              {s.lessons.map((l, j) => (
                                ...
                                >
                                  <div className="w-8 h-8 bg-muted border border-border flex items-center justify-center shrink-0">
                                    <Play size={11} className="text-text-muted ml-0.5" />
                                  </div>
                                  <span className="text-text-muted text-sm hover:text-text-main transition-colors">{l}</span>
                                  <span className="ml-auto text-label font-mono text-text-muted">12:30</span>
                                </Link>
                              ))}
                            </div>
                          </motion.div>
            ...
            <RevealBlock>
              <div className="p-8 border border-border bg-muted-light flex gap-6 mb-16">
                ...
                <div>
                  <div className="text-tiny font-mono text-gold tracking-widest uppercase mb-1">Instructor</div>
                  <div className="text-text-main font-bold text-xl mb-2">{course.instructor}</div>
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-28 border border-border bg-muted-light">
              <div className="relative aspect-video overflow-hidden">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <Link href="/lesson-player">
                    <button
                      className="w-16 h-16 bg-text-main/10 backdrop-blur-md border border-white/20 flex items-center justify-center hover:bg-gold/20 hover:border-gold/40 transition-all"
                    >
              ...
              <div className="p-8">
                <div className="text-text-main font-extrabold text-4xl mb-1">€{course.price}</div>
                <div className="text-label font-mono text-text-muted tracking-widest mb-8">Lifetime access · All devices</div>
